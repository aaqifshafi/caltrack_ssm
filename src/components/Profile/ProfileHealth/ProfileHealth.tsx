...
import {
  calculateBmr,
  calculateCalories,
  clearUserHealthNumberData,
  editUserHealthData,
  editUserHealthNumberData,
...
const ProfileHealth: FC = (): ReactElement => {
  ...
  const profileBMI =
    userData!.health?.weightMetric && userData!.health?.heightMetric
      ? userData!.health?.weightMetric /
        (userData!.health?.heightMetric / 100) ** 2
      : null;
  ...
  const {
    getInputProps: getInputPropsWeight,
    getIncrementButtonProps: getIncrementButtonPropsWeight,
    getDecrementButtonProps: getDecrementButtonPropsWeight,
  } = useNumberInput({
    step: 0.5,
    min: 1,
  });
  ...
  const {
    getInputProps: getInputPropsHeight,
    getIncrementButtonProps: getIncrementButtonPropsHeight,
    getDecrementButtonProps: getDecrementButtonPropsHeight,
  } = useNumberInput({
    step: 0.1,
    min: 1,
  });
  ...
  const {
    getInputProps: getInputPropsWater,
    getIncrementButtonProps: getIncrementButtonPropsWater,
    getDecrementButtonProps: getDecrementButtonPropsWater,
  } = useNumberInput({
    step: 25,
    min: 500,
  });
        ...
        >
          ...
          <Tbody>
            <Tr>
              ...
              <Td>
                {!userData!.health?.gender ? (
                  "No entry"
                ) : (
                  <Badge
                    fontSize="0.8em"
                    colorScheme={
                      userData!.health.gender === Gender.male ? "blue" : "pink"
                    }
                  >
                    {userData!.health.gender}
                  </Badge>
            ...
            <Tr>
              ...
              <Td>
                {!userData!.health?.weightMetric ? (
                  ...
                  <>
                    {isMetric
                      ? userData!.health?.weightMetric
                      : userData!.health?.weightImperial}{" "}
                    <Badge
                      fontSize="0.8em"
                      colorScheme={isMetric ? "blue" : "pink"}
                    >
                      {isMetric ? "kg" : "lbs"}
                    </Badge>
                  </>
              ...
              <Td>
                <VStack>
                  <HStack>
                    <Button {...decWeight} colorScheme="facebook">
                      -
                    </Button>
                    <Input
                      {...inputWeight}
                      ...
                      bg={inputColor}
                    />
                    <Button {...incWeight} colorScheme="facebook">
                      +
                    </Button>
                  </HStack>
                  <ButtonGroup>
                    ...
                    <Button
                      variant="ghost"
                      colorScheme="orange"
                      onClick={() => handleClearNumberData("weight")}
                    >
                      Clear
                    </Button>
                  </ButtonGroup>
            ...
            <Tr>
              ...
              <Td>
                {!userData!.health?.heightMetric ? (
                  ...
                  <>
                    {isMetric
                      ? userData!.health?.heightMetric
                      : userData!.health?.heightImperial}{" "}
                    <Badge
                      fontSize="0.8em"
                      colorScheme={isMetric ? "blue" : "pink"}
                    >
              ...
              <Td>
                <VStack>
                  <HStack>
                    <Button {...decHeight} colorScheme="facebook">
                      -
                    </Button>
                    <Input
                      {...inputHeight}
                      ...
                      bg={inputColor}
                    />
                    <Button {...incHeight} colorScheme="facebook">
                      +
                    </Button>
                  </HStack>
                  <ButtonGroup>
                    ...
                    <Button
                      variant="ghost"
                      colorScheme="orange"
                      onClick={() => handleClearNumberData("height")}
                    >
                      Clear
                    </Button>
                  </ButtonGroup>
              ...
              <Td>
                <HStack>
                  <VStack>
                    <Text>
                      {profileBMI
                        ? profileBMI.toFixed(1)
                        : "Insufficient data for BMI calculation."}
                    </Text>
                    {!profileBMI && (
                      <Text color="gray" fontSize="0.8em">
                        <i>Weight and Height data needed.</i>
                      </Text>
                    )}
                  </VStack>
                  <Badge colorScheme={bmiData.color} fontSize="0.8em">
                    {bmiData.category}
                  </Badge>
                </HStack>
              </Td>
              ...
              <Td>
                <HStack>
                  <VStack>
                    <Text>
                      {profileBmr
                        ? profileBmr.toFixed(1)
                        : "Insufficient data for BMR calculation."}
                    </Text>
                    {!profileBmr && (
                      <HStack>
                        <Text color="gray" fontSize="0.8em">
                          <i>
                            Date of Birth, Gender, Weight and Height data
                            needed.
                          </i>
                        </Text>
                        <Tooltip
                          label={`Date of Birth is found in 'Details' tab`}
                        >
                          <span>
                    ...
                    >
                      {Object.values(ActivityLevel).map((activity) => (
                        <option key={activity} value={activity}>
                          {ACTIVITY_LEVEL_DATA[activity].description}
                        </option>
                      ))}
                    </Select>
                  </HStack>
              ...
              <Td>
                ...
                {profileCalories > 0 && profileCalories <= 1200 && (
                  <Text mt="2" color="red" fontStyle="italic" fontSize="0.8em">
                    It is not recommended to consume less than 1200 calories a
                    day. Please consult a doctor!
                  </Text>
                )}
              </Td>
              <Td>
              ...
              <Td pl="0">
                <VStack>
                  <HStack>
                    ...
                    >
                      {Object.values(WeightGoal).map((wGoal) => (
                        <option key={wGoal} value={wGoal}>
                          {WEIGHT_GOAL_DATA[wGoal].description}
                        </option>
                      ))}
                    </Select>
                  </HStack>
            ...
            <Tr>
              <Td>
                ...
                {recommendedWaterTargetMetric &&
                  recommendedWaterTargetImperial &&
                  (isMetric ? (
                    <Text mt="2" fontStyle="italic" fontSize="0.8em">
                      (recommended water intake:{" "}
                      {recommendedWaterTargetMetric.toFixed(0)} ml)
                    </Text>
                  ) : (
                    <Text mt="2" fontStyle="italic" fontSize="0.8em">
                      (Recommended water intake:{" "}
                      {recommendedWaterTargetImperial.toFixed(1)} fl oz)
                    </Text>
                  ))}
              </Td>
              <Td>
                {!userData!.health?.waterTargetMetric ? (
                  ...
                  <>
                    {isMetric
                      ? userData!.health?.waterTargetMetric
                      : userData!.health?.waterTargetImperial}{" "}
                    <Badge
                      fontSize="0.8em"
                      colorScheme={isMetric ? "blue" : "pink"}
                    >
              ...
              <Td>
                <VStack>
                  <HStack>
                    <Button {...decWater} colorScheme="facebook">
                      -
                    </Button>
                    <Input
                      {...inputWater}
                      ...
                      bg={inputColor}
                    />
                    <Button {...incWater} colorScheme="facebook">
                      +
                    </Button>
                  </HStack>
                  <ButtonGroup>
                    ...
                    <Button
                      variant="ghost"
                      colorScheme="orange"
                      onClick={() => handleClearNumberData("waterTarget")}
                    >
                      Clear
                    </Button>
                  </ButtonGroup>
