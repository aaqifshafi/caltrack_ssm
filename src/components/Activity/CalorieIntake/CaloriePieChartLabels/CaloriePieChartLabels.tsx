...
const CaloriePieChartLabels: FC<{
  calorieLog: { [key: string]: number };
  recommendedCalories: number;
}> = ({ calorieLog, recommendedCalories }): ReactElement => {
  const calorieIntake = Object.values(calorieLog).reduce(
    (acc, curr) => acc + curr,
    0
  );
  ...
  return (
    <VStack align="start" w="100%" p={2}>
      <Heading as="h3" size="md" bottom={20} textAlign="center" w="100%">
        Calorie Intake
      </Heading>
      <HStack w="100%" justify="space-evenly" p={4} rounded="lg">
        {calorieIntake > recommendedCalories - 40 &&
        calorieIntake < recommendedCalories + 40 ? (
