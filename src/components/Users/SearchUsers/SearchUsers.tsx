...
const SearchUsers: FC<{ hasBorder?: boolean }> = ({ hasBorder = false }) => {
    ...
    getAllUsers()
      ...
      .then((users) => {
        ...
        return users.filter((user) => {
          ...
          return (
            user.handle.toLowerCase().includes(formattedSearchTerm) ||
  ...
  return (
    <VStack w="100%" align="center">
      ...
      <Modal
        isOpen={searching && !!foundUsers.length}
        onClose={() => {
          onClose();
          setSearching(false);
        }}
        ...
        <ModalContent maxW={{ base: "100%", md: "70%" }}>
          <ModalHeader>
            <Heading as="h3" size="md">
              Users found for &apos;{searchTerm}&apos; and between {fromAge} to{" "}
              {toAge} years old:{" "}
            </Heading>
            <ModalCloseButton />
          </ModalHeader>
