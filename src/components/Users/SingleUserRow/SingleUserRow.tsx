...
const SingleUserRow: FC<{ user: IUserData }> = ({ user }): ReactElement => {
  ...
  const roleToDisplay =
    currUser.role === UserRoles.WantAdmin || currUser.role === UserRoles.Base
      ? "user"
      : currUser.role;

  const handleAddFriend = () => {
    setLoadingBtn(true);
    sendFriendRequest(userData!.handle, user.handle).then(() =>
      setLoadingBtn(false)
    );
  };
  ...
  const handleRejectFriendRequest = () => {
    setLoadingBtn(true);
    resolveRequestByRecipient(userData!.handle, user.handle).then(() =>
      setLoadingBtn(false)
    );
  };
  ...
  const handleWithdrawFriendRequest = () => {
    setLoadingBtn(true);
    resolveRequestBySender(userData!.handle, user.handle).then(() =>
      setLoadingBtn(false)
    );
  };
  ...
  const handleRemoveFriend = () => {
    setLoadingBtn(true);
    unFriend(userData!.handle, user.handle).then(() => setLoadingBtn(false));
  };
  ...
  useEffect(() => {
    return onValue(ref(db, `users/${userData!.handle}`), (snapshot) => {
      ...
      setIsFriend(
        data.friends ? Object.keys(data.friends).includes(user.handle) : false
      );
      setRequestSent(
        data.sentFriendRequests
          ? Object.keys(data.sentFriendRequests).includes(user.handle)
          : false
      );
      setRequestReceived(
        data.receivedFriendRequests
          ? Object.keys(data.receivedFriendRequests).includes(user.handle)
          : false
      );
  ...
  return (
    <Tr>
      ...
      <Td>
        <Badge fontSize="14px" p="3px 10px" borderRadius="8px">
          {roleToDisplay}
        </Badge>
      </Td>
      {!handle ? (
