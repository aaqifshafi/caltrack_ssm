...
import { ActivityLevel, Gender, UserRoles, WeightGoal } from "../common/enums";
...
export const editUserHealthNumberData = ({
  ...
  const coeffMetricToImperial =
    propKey === "weight" ? 2.2 : propKey === "height" ? 0.033 : 0.0338;
 ...
 */
export const clearUserHealthNumberData = ({
  handle,
  propKey,
}: {
  handle: string;
  ...
  return update(ref(db, `users/${handle}`), {
    ...
    switch (newRole) {
      case UserRoles.Premium:
        ...
        break;
      case UserRoles.UltraPremium:
        message = "Congratulations! You now have Ultra Premium access!";
        break;
      case UserRoles.Admin:
        message = "You are now an admin!";
        break;
      case UserRoles.Blocked:
        message = "You have been blocked from using the service.";
        break;
      default:
        message = "Your role has been updated.";
    }
    addNotification(handle, message);
  });
};
